...
section .bss
    ...
    read_pixel resq 1                     ; Pixel read in from file
...
read_integer:
    xor r13, r13                         ; Clear rax for the result
    .next_digit:
        xor rax, rax 
        mov al, byte [rdi]               ; Load current character
        ...
        imul r13, 10                ; Multiply current result by 10
...
is_in_set:
    .loop:
        mov bl, byte [newline]
        cmp al, bl
        jz .in_set                   ; End if null terminator
        mov bl, byte [space]
        cmp al, bl
        jz .in_set
        mov bl, byte [tabspace]
        cmp al, bl
        jz .in_set
        mov bl, byte [carriage]
        cmp al, bl
        jz .in_set
        jmp .not_in_set
